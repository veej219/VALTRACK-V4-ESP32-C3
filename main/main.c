    ...
    .Fields =
        {
            ...
            "https://script.google.com/macros/s/AKfycbyPk8a2zErZelfPFr8u6XD-zGFudhw3I9iQ7WbBjfy9t2OxC0-1ACG28c6Y3fKa0jev/exec", // char HTTPURL[150];,
            "Your-Api-Key: 1234456789",                                                                                         /// char HTTPKey[100];
            "giffgaff.com",                                                                                                     //"iot.1nce.net",//char APNName[20];
            "gg",                                                                                                               // char APNUsername[20];
            "p",                                                                                                                // char APNPassword[20];
...
char XHTTP_Request(char *pFilename, unsigned char pingtype)
{
    ...
    //   RESEND_HTTP: // TBD
    if (SystemState == State_ConnectedState)
        return 0; // Return and idle for proper configuration and prevent EEPROM access
    WakeUp();
    if (DeviceStatus == 0)
        return 0;
    //    SOS = gpio_get_level(GPIO_SOS);
    ...
    if (SendATCommand(cmdstr, "DOWNLOAD", "ERROR", 10) != 1)
        goto exit;
...
#ifdef TIMER_ONLY_WAKEUP
                        SystemTimer = 0;
                        while (SystemTimer < 300 && GPSStatus != 'A')
                        {
                            LPUARTTimer = 0;
                        };
                        osDelay(5000);
